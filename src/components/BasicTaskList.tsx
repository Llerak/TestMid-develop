...
import { plusSquareIcon, minorSquareIcon } from "../assets/Icons";
...
const BasicTaskList = () => {
	...
	const [borderStyle] = useState("border rounded-md w-3/5");
          ...
          >
            <div className="flex p-2 w-full h-10">
              <i className="cursor-pointer" onClick={() => setMenuDisplay(!menuDisplay)}>
                {menuDisplay?minorSquareIcon:plusSquareIcon}
              </i>
              <input
                  type="text"
              ...
              />
              <span
                  className="absolute h-6 w-full flex items-center pointer-events-none ml-[29.99px] font-serif"
                  ref={spanRef}
              ></span>
            </div>
            {menuDisplay && (
